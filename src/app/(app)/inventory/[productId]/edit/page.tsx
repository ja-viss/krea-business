...
import { ChevronLeft, Loader2, Scale, Camera, ScanLine, Save, Wand2, Link2, Image as ImageIcon, Briefcase, Boxes, Coins, TrendingUp } from 'lucide-react';
...
export default function EditProductPage() {
  ...
  const handleBarcodeScan = (scannedCode: string) => {
    ...
    toast({ title: 'Código Escaneado', description: `Actualizado: ${scannedCode}` });
  ...
  const handleAutoSearchImage = async () => {
    ...
    try {
      ...
      setImgKey(timestamp);
      toast({ title: 'Imagen Actualizada', description: `Referencia cargada para: ${watchName}` });
    } catch (e) {
                ...
                <Card className='border-none shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-2xl overflow-hidden bg-white'>
                  <CardHeader className='bg-slate-50/50 border-b border-slate-100 py-3'>
                    <CardTitle className='text-[11px] font-black uppercase flex items-center gap-2 text-slate-700 tracking-wider'>
                        <Briefcase className="h-3.5 w-3.5 text-primary" /> Ficha de Identidad
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-5 pt-5">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <FormField control={form.control} name="name" render={({ field }) => (
                          <FormItem className='md:col-span-2'>
                            ...
                            <div className="flex gap-2">
                              <FormControl><Input className="h-11 text-base font-bold rounded-xl border-slate-200" {...field} /></FormControl>
                              <Button type="button" variant="secondary" className="h-11 px-3 rounded-xl bg-amber-500 text-white hover:bg-amber-600 shrink-0" onClick={handleAutoSearchImage} disabled={searchingImage}>
                        ...
                        <FormField control={form.control} name="category" render={({ field }) => (
                            ...
                            <FormControl><Input className="h-11 font-bold rounded-xl border-slate-200" {...field} /></FormControl></FormItem>
                        )} />
                            ...
                            <div className="flex gap-2">
                                <FormControl>
                                    <div className="relative flex-1 group">
                                        ...
                                        <Input className="h-12 pl-11 font-mono font-bold rounded-xl border-slate-200" {...field} />
                                    </div>
                                </FormControl>
                ...
                <Card className='border-none shadow-2xl rounded-2xl overflow-hidden bg-white'>
                  ...
                  <CardContent className='pt-6 space-y-5 px-6'>
                      ...
                      <FormField control={form.control} name="taxRate" render={({ field }) => (
                          <FormItem><FormLabel className='text-[10px] font-black uppercase text-slate-400'>Carga Fiscal (IVA)</FormLabel><Select onValueChange={(v) => field.onChange(parseFloat(v))} value={String(field.value)}><FormControl><SelectTrigger className='h-11 font-bold rounded-xl border-slate-100 bg-slate-50/50'><SelectValue /></SelectTrigger></FormControl>
