    ...
    <div className="flex flex-1 flex-col bg-[#f8fafc]">
      <main className="flex-1 space-y-6 p-4 pt-6 md:p-8 max-w-[1300px] mx-auto w-full">
        <PageHeader
          ...
          description="Gestión maestra de catálogo con validación fiscal."
          actions={
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-12 items-start">
              ...
              <div className="space-y-6 lg:col-span-8">
                  ...
                  <CardContent className="space-y-5 pt-5">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <FormField control={form.control} name="productType" render={({ field }) => (
                            <FormItem><FormLabel className='text-[10px] font-black uppercase text-slate-500'>Tipo de Gestión</FormLabel>
                ...
                <Card className='border-none shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-2xl overflow-hidden bg-white'>
                  <CardHeader className='bg-slate-50/50 border-b border-slate-100 py-3'>
                    <CardTitle className='text-[11px] font-black uppercase flex items-center gap-2 text-slate-700 tracking-wider'>
                        <Boxes className="h-3.5 w-3.5 text-primary" /> Almacén y Control Físico
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-6 pt-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <FormField control={form.control} name="baseUnit" render={({ field }) => (
                            <FormItem>
                                <FormLabel className='text-[10px] font-black uppercase text-slate-500'>Unidad de Venta</FormLabel>
                        ...
                        <FormField control={form.control} name="isWeightable" render={({ field }) => (
                            ...
                            <div className="space-y-0.5">
                                ...
                                <p className='text-[7px] font-bold text-slate-400 italic'>Soporta Balanza</p>
                            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <FormField control={form.control} name="minStock" render={({ field }) => (
                          <FormItem>
                              <FormLabel className='text-[10px] font-black uppercase text-red-500'>Mínimo (Alerta de Stock)</FormLabel>
                    ...
                    <FormField control={form.control} name="barcode" render={({ field }) => (
                        <FormItem>
                            <FormLabel className='text-[10px] font-black uppercase text-slate-500'>Código de Barras / SKU</FormLabel>
                            <div className="flex gap-2">
                                <FormControl>
                                    <div className="relative flex-1 group">
                                        <ScanLine className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-primary opacity-30" />
                                        <Input placeholder="Escriba o escanee el código" className="h-12 pl-11 font-mono font-bold rounded-xl border-slate-200" {...field} value={field.value ?? ''} />
                                    </div>
                                </FormControl>
                ...
                <Card className='border-none shadow-2xl rounded-2xl overflow-hidden bg-white'>
                  <div className='bg-gradient-to-r from-primary to-purple-600 p-4 text-white'>
                    <div className='flex justify-between items-center'>
                        <CardTitle className='text-[10px] font-black uppercase flex items-center gap-2 italic tracking-widest'><Coins className='h-4 w-4' /> Valorización Fiscal</CardTitle>
                  ...
                  <CardContent className='pt-6 space-y-5 px-6'>
                      <FormField control={form.control} name="cost" render={({ field }) => (
                          <FormItem><FormLabel className='text-[10px] font-black uppercase text-slate-400'>Costo de Compra (Bs.)</FormLabel><FormControl><Input type="number" step="0.01" className='h-11 border-slate-100 font-bold text-center text-base rounded-xl bg-slate-50/50' {...field} value={field.value ?? 0} /></FormControl><FormMessage /></FormItem>
                      )} />

                      <FormField control={form.control} name="price" render={({ field }) => (
                          <FormItem>
                            <div className='flex justify-between items-end mb-1'><FormLabel className='text-[10px] font-black uppercase text-primary'>Precio de Venta (PVP)</FormLabel><Badge variant="secondary" className="font-black text-[9px] text-green-600 bg-green-50 border-green-100 px-2 py-0.5 rounded-full flex gap-1"><TrendingUp className='h-3 w-3' /> {margin}%</Badge></div>
                      ...
                      <FormField control={form.control} name="taxRate" render={({ field }) => (
                          <FormItem><FormLabel className='text-[10px] font-black uppercase text-slate-400'>Tasa de IVA</FormLabel><Select onValueChange={(v) => field.onChange(parseFloat(v))} defaultValue={String(field.value)}><FormControl><SelectTrigger className='h-11 font-bold rounded-xl border-slate-100 bg-slate-50/50'><SelectValue /></SelectTrigger></FormControl>
                          <SelectContent className='rounded-xl'><SelectItem value="0.16" className="font-bold py-2 uppercase text-[10px]">IVA General (16%)</SelectItem><SelectItem value="0.08" className="font-bold py-2 uppercase text-[10px]">IVA Reducido (8%)</SelectItem><SelectItem value="0" className="font-bold py-2 uppercase text-[10px]">Exento (0%)</SelectItem></SelectContent></Select><FormMessage /></FormItem>
                      )} />
