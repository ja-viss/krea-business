...
import { Truck, PlusCircle, FileText, Search, Loader2, Calendar, ShoppingBag, Eye, Clock, CheckCircle2, AlertCircle } from 'lucide-react';
        ...
        <div className="flex flex-1 flex-col">
            <main className="flex-1 space-y-6 p-4 pt-6 md:p-8">
                <PageHeader 
                    ...
                    description="Planifica la reposición de stock y monitorea tus pedidos en tránsito."
                    actions={
                        <Button asChild className="font-black uppercase shadow-lg shadow-primary/20 h-11 px-6">
                ...
                <div className="grid gap-4 md:grid-cols-3">
                    <Card className="border-2 shadow-sm bg-blue-50/10 border-blue-100">
                        <CardHeader className="pb-2">
                            <CardTitle className="text-[10px] font-black uppercase text-blue-600 flex items-center gap-2">
                                <Clock className="h-4 w-4" /> Pedidos Pendientes
                            </CardTitle>
                        </CardHeader>
                        <CardContent>
                    ...
                    <Card className="bg-primary/5 border-primary/20 border-2">
                        <CardHeader className="pb-2">
                            <CardTitle className="text-[10px] font-black uppercase text-primary flex items-center gap-2">
                                <ShoppingBag className="h-4 w-4" /> Inversión Comprometida
                            </CardTitle>
                        </CardHeader>
                        <CardContent>
                    ...
                    <Card className="border-2 border-dashed bg-muted/20">
                        <CardHeader className="pb-2">
                            <CardTitle className="text-[10px] font-black uppercase opacity-60">Control Logístico</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <p className="text-[9px] font-bold text-muted-foreground italic leading-tight">
                                El stock de estos pedidos aparecerá como "En Tránsito" en tu inventario hasta que confirmes la recepción.
                            </p>
                        </CardContent>
                    </Card>
                </div>
                ...
                <div className="flex items-center gap-4">
                    <div className="relative flex-1 max-w-sm">
                        ...
                        <Input 
                            placeholder="Buscar REQ o proveedor..." 
                ...
                <Card className="border-2 shadow-xl overflow-hidden">
                    <CardHeader className="bg-muted/10 border-b">
                        <CardTitle className="text-lg font-black uppercase flex items-center gap-2 italic tracking-tight">
                            <Truck className="h-5 w-5 text-primary" /> Diario de Adquisiciones
                        </CardTitle>
                    </CardHeader>
                    <CardContent className="p-0">
                        <Table>
                            <TableHeader className="bg-muted/50">
                                <TableRow>
                                    <TableHead className="font-black text-[10px] uppercase pl-6 py-4">Nº Orden</TableHead>
                                    <TableHead className="font-black text-[10px] uppercase">Proveedor</TableHead>
                                    <TableHead className="font-black text-[10px] uppercase">Fecha Emisión</TableHead>
                                    ...
                                    <TableHead className="text-right font-black text-[10px] uppercase pr-6">Monto Pedido</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {loading ? (
                                    ...
                                    filtered.map((p) => (
                                        <TableRow key={p._id} className="hover:bg-primary/[0.02] cursor-pointer transition-colors group" onClick={() => router.push(`/purchases/${p._id}`)}>
                                            <TableCell className="pl-6 font-mono text-xs font-bold text-primary">REQ-{String(p.orderNumber).padStart(4, '0')}</TableCell>
                                            <TableCell className="font-black uppercase text-[10px] truncate max-w-[150px]">{p.vendor}</TableCell>
                                            <TableCell className="text-xs font-medium opacity-60">{format(new Date(p.issuedDate), 'dd/MM/yy')}</TableCell>
                                            <TableCell className="text-xs font-bold">{format(new Date(p.expectedDeliveryDate), 'dd/MM/yy')}</TableCell>
