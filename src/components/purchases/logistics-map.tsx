...
import { Truck, Store, MapPin, Loader2, Factory, PackageCheck, Navigation, Clock } from 'lucide-react';
...
export function LogisticsMap({ status, storeCoords, providerCoords, vendorName }: LogisticsMapProps) {
  ...
  const telemetria = useMemo(() => {
    if (!storeCoords || !providerCoords) return { distance: 0, time: 0 };
    ...
    // Asumiendo velocidad promedio de 60km/h
    ...
    return { 
        ...
        time: minutes,
        currentLat: providerCoords.lat + (storeCoords.lat - providerCoords.lat) * (progress / 100),
        currentLng: providerCoords.lng + (storeCoords.lng - providerCoords.lng) * (progress / 100)
    };
      ...
      <CardContent className="p-0 flex-1 relative">
        ...
        >
          ...
          <Marker position={[providerCoords.lat, providerCoords.lng]} icon={providerIcon}>
            <Popup><span className="font-bold text-[10px]">ORIGEN: {vendorName}</span></Popup>
          </Marker>

          <Marker position={[storeCoords.lat, storeCoords.lng]} icon={storeIcon}>
            <Popup><span className="font-bold text-[10px]">DESTINO: TU ALMACÉN</span></Popup>
          </Marker>

          {(status === 'In Transit' || status === 'Delivered') && (
            <Marker position={[telemetria.currentLat, telemetria.currentLng]} icon={truckIcon}>
                <Popup><span className="font-bold text-[10px]">UBICACIÓN ACTUAL CARGA</span></Popup>
            </Marker>
          )}
        ...
        <div className="absolute bottom-4 left-4 right-4 z-[500] grid grid-cols-2 gap-2">
            <div className="bg-white/95 backdrop-blur p-3 rounded-xl border-2 border-primary/20 shadow-xl flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className='text-xs font-black uppercase'>{telemetria.time > 60 ? `${Math.floor(telemetria.time/60)}h ${telemetria.time%60}min` : `${telemetria.time} min`}</p>
                </div>
            </div>
            <div className="bg-black text-white p-3 rounded-xl border-2 border-white/10 shadow-xl flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className='text-xs font-black uppercase'>{telemetria.distance} Kilómetros</p>
                </div>
            </div>
        </div>
      </CardContent>
